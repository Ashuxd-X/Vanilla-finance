...
from bot.utils.api_checker import check_base_url
...
from bot.utils.websocket_manager import WebSocketHandler
...
class Tapper:
    ...
    async def get_tg_web_data(
        ...
        try:
            ...
            logger.error(
                f"<light-yellow>{self.session_name}</light-yellow> | Session Revoked! Login Again...")
            await asyncio.sleep(delay=3)
        except UserDeactivatedBan:
            logger.error(
                f"<light-yellow>{self.session_name}</light-yellow> | The TG Account Has Been Banned!")
            await asyncio.sleep(delay=3)
    ...
    async def join_tg_channel(
        ...
        async with self.tg_client:
            try:
                ...
                logger.error(
                    f"<light-yellow>{self.session_name}</light-yellow> | Session Is Revoked! Try To Login Again")
                await asyncio.sleep(delay=3)
            except UserDeactivatedBan:
                logger.error(
                    f"<light-yellow>{self.session_name}</light-yellow> | Your Telegram account has been banned")
                await asyncio.sleep(delay=3)
            except UserRestricted as e:
                logger.error(
                    f"<light-yellow>{self.session_name}</light-yellow> | Your account is restricted. Details: {e}", exc_info=True)
            ...
            while True:
                ...
                try:
                    ...
                    if can_run:
                        ...
                        await websocket_manager.connect_websocket(ws_url, subscribe_message, user_agent)
                        ...
                        if settings.UPGRADE_LEVEL_WITH_SUGAR:
                        ...
                        await websocket_manager.close_websocket()
